...
async function fetchWithRetry(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      const res = await fetch(url, {
        ...
        headers: {
          "User-Agent": "StaticMapOSMGenerator/1.0 (https://github.com/yourusername/staticmap-osm-generator)",
        },
      });
      ...
      if (lastError.name === 'AbortError') {
        ...
        logger.warn({ url, attempt, error: lastError.message }, 'Tile fetch failed');
      }
  ...
  throw lastError || new Error(`Failed to fetch tile after ${maxRetries} attempts`);
}
