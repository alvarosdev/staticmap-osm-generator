...
import { generateSingleTileMap, preloadMarkerImages } from '@/core/tile.js';
...
// Font will be registered at runtime if present
...
async function handleMapRequest(
	...
	if (await file.exists()) {
		...
		const pngBuffer = await generateSingleTileMap(
			{ lat, lon, zoom, markerName, anchorName, outputScale: scale },
			CONFIG
		);
...
(async () => {
	...
	try {
		const fontPath = 'assets/fonts/CabinCondensed-SemiBold.ttf';
		if (existsSync(fontPath)) {
			registerFont(fontPath, { family: 'Cabin Condensed' });
		}
	} catch {}
	try {
		await preloadMarkerImages(CONFIG, 32);
	} catch {}
	...
	Bun.serve({
		...
		async fetch(request, _server) {
			...
			if (url.pathname === '/map' && request.method === 'GET') {
				...
				const latValidation = validateNumber(
				...
				const lonValidation = validateNumber(
				...
				const lat = latValidation.value as number;
				const lon = lonValidation.value as number;
				const zoom = zoomValidation.value as number;
				// Optional scale param (1..4)
				const scaleValidation = validateInteger(
				...
				const scale =
					scaleValidation.valid && scaleValidation.value
						? scaleValidation.value
						: 1;
				...
				if (markerParam) {
					const markerExists = availableMarkers.some(
						(m) => m.name === markerParam
					);
					if (!markerExists) {
						logger.warn({ markerParam }, 'Invalid marker');
						return new Response(
							`marker must be one of: ${availableMarkers.map((m) => m.name).join(', ') || 'none configured'}`,
							{
								status: 400,
								headers: {
									'Content-Type': 'text/plain',
				...
				if (!anchorName) {
					const foundMarker = availableMarkers.find(
						(m) => m.name === markerName
					);
				...
				if (!anchorValid) {
					...
					return new Response(
						`anchor must be one of: ${availableAnchors.map((a) => a.name).join(', ') || 'none configured'}`,
						{
							status: 400,
							headers: {
								'Content-Type': 'text/plain',
